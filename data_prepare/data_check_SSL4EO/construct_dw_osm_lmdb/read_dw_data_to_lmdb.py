...
def write_original_tif_to_lmdb_in_small_patch(tif_dir, lmdb_dir, map_size_G, patch_size, patch_bias=0):
    env = lmdb.open(lmdb_dir, map_size=int(map_size_G*1024**3), writemap=True)
    with env.begin(write=True) as txn:
        subdirs = os.listdir(tif_dir)
        fnames = [f[3:] for f in os.listdir(os.path.join(tif_dir, 'image_s1')) if f.endswith('.tif')]
        for subdir in subdirs:
            ...
            if 's1' in subdir:
                for fi, fname in tqdm(enumerate(fnames), desc=desc_head):
                    ...
                    imgs = pad_and_crop_image_BWH(img, patch_size, patch_bias)
                    # # # # # pad and crop the image # # # # #
...
def pad_and_crop_image_BWH(img, patch_size=256, patch_bias=0):
    ...
    n_patch = int((512-patch_bias)/patch_size)
    imgs = []
...
if __name__ == '__main__':
    save_type = 'smaller'  # original, small, smaller
    patch_size = 102
    patch_bias = 1
    pdir = 'DW'
    splits = ['osm_test', 'osm_train', 'dw_train', 'dw_test']
    map_size_Gs = [5, 15, 31.5, 3]   # 106, 1666, 1219, 3574, 340
    if save_type == 'smaller':
        lmdb_dir_name = 'data_lmdb_smaller'
    elif save_type == 'small':
        lmdb_dir_name = 'data_lmdb_small'
    else:
        lmdb_dir_name = 'data_lmdb'
    ...
    for i, sp in enumerate(splits):
        ...
        if 'small' in save_type:
            write_original_tif_to_lmdb_in_small_patch(tif_dir, lmdb_dir, map_size_G, patch_size, patch_bias)
        else:
            write_original_tif_to_lmdb(tif_dir, lmdb_dir, map_size_G)