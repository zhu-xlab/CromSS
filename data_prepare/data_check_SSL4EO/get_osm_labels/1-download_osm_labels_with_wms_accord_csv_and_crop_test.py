...
and save as tif for the test set
...
def get_and_save_single_label_image(bbox, dst_bbox, dst_regsize, dst_crs, color_mapping, out_path):
    ...
    try:
        ...
        if np.sum(osm>0)>0:
            ...
            return {'dw_id':dw_id, 'err':0}
        else:
            return {'dw_id':dw_id, 'err':'No label in this area!'}
    except Exception as e:
        return {'dw_id':dw_id, 'err':f'Pulling error: {e}'}
...
if __name__ == '__main__':
    ...
    save_dir = '/p/scratch/hai_dm4eo/liu_ch/test/data_test/download_osm'
    fp_csv = '/p/scratch/hai_dm4eo/liu_ch/test/data_test/center_voids_test.csv'
    fp_err = os.path.join(save_dir, 'Error_messages_for_osm_data_pulling.csv')
    n_core = 0
    ...
    bboxes = [[meta.loc[i,f'bbox{j}'] for j in range(1,5)] for i in meta.index]
    dst_bboxes = [[meta.loc[i,f'bound{j}'] for j in range(1,5)] for i in meta.index]
    dst_crses = [meta.loc[i,'crs'] for i in range(n_sam)]
    dst_regsizes = [[meta.loc[i,'width'],meta.loc[i,'height']] for i in meta.index]
    out_paths = [os.path.join(save_dir, f"osm_{meta.loc[i,'dw_id'][3:]}.tif") for i in meta.index]
    ...
    num_processes = n_core if n_core>0 else n_CPUs
    
    outputs = []
    if num_processes>1:
        # Create a pool of worker processes
        print(f'{num_processes} cores out of {n_CPUs} are used!')
        ...
        for d in [data[1]]:
            ...
            if r['err']!=0: 
                outputs.append([r['dw_id'],r['err']])
    print(f"{len(outputs)} files have error message.")
    
    # write to excel
    df = pd.DataFrame(outputs, columns=['dw_id', 'error_message'])
    # Write the DataFrame to an Excel file
    df.to_csv(fp_err, index=False)
    
    t1 = datetime.datetime.now().replace(microsecond=0)
    print(f"Finished! Used {t1-t0}.")
