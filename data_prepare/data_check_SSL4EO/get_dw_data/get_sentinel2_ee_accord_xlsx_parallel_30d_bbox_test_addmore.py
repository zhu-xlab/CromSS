...
def clip_img_with_bbox(bbox, crs, t_start, t_end, download_l2a=False):
    ...
    # s2 collection
    if download_l2a:
        s2 = ee.ImageCollection('COPERNICUS/S2_SR') # ('COPERNICUS/S2_SR_HARMONIZED') # ('COPERNICUS/S2_SR')
    else:
        s2 = ee.ImageCollection('COPERNICUS/S2')
    ...
    # get data array
    image = np.array(s2_image_sample.getInfo()['properties']['array'])
    cloud_coverage_ = s2_image.get('CLOUDY_PIXEL_PERCENTAGE').getInfo()
    assert cloud_coverage_==cloud_coverage, "Cloud coverage is inconsistent!"
...
def get_single_s2_data_from_ee(ds, download_l2a=False):  
    ...
    date_obj = datetime.strptime(date_str, "%Y%m%d") + relativedelta(years=1) #+ timedelta(days=TIME_BUFF*5)
    date_obj_after = date_obj + timedelta(days=TIME_BUFF) # + timedelta(years=-1)
    date_obj_before = date_obj + timedelta(days=-TIME_BUFF) # + timedelta(years=-1)
    ...
    # fetch data
    try:
        # image, cinfo = clip_img_with_bbox(bbox, crs, formatted_date_string, formatted_date_string_after, download_l2a)
        try:
            # current day + 15 days
            image, cinfo = clip_img_with_bbox(bbox, crs, formatted_date_string, formatted_date_string_after, download_l2a)
        except:
            # current day - 15 days
            image, cinfo = clip_img_with_bbox(bbox, crs, formatted_date_string_before, formatted_date_string, download_l2a)
        return (image, cinfo)
...
def get_s2_for_each_tif(args):
    ...
    # get image from ee
    image_ = get_single_s2_data_from_ee(ds, download_l2a)
    if type(image_) is str:
        err_msg = image_
    else:
        image, cloud_coverage = image_
        # display(did, cloud_coverage)  # ee.Number
...
if __name__ == '__main__':
    label_file_path = '/p/project1/hai_ws4eo/liu_ch/DW/test/data_test/center_voids_test_lack_in_s2a_v2.csv'
    save_dir = '/p/project1/hai_ws4eo/liu_ch/DW/test/data_test/download_s2a_lack_v2'
