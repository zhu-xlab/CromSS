...
def get_s12_for_each_tif(args):
    ...
    if success:
        for img, tag, it in zip((s1_image, s2_image), ('s1','s2'), (np.float32, np.int16)):
            tif_path = os.path.join(save_dir, f'{tag}_{did[3:]}.tif')
...
if __name__ == '__main__':
    # label_file_path = '/p/scratch/hai_dm4eo/liu_ch/test/data_test/center_voids_test.csv'
    # save_dir = '/p/scratch/hai_dm4eo/liu_ch/test/data_test/download_s2'
    ...
    data = [(df.loc[i], save_dir) for i in df.index]
    ...
    get_s2_for_each_split = get_s12_for_each_tif
    err_msgs = []
    dw_ids = []
    if num_processes>1:
