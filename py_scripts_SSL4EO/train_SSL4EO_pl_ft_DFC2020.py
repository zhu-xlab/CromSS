...
from py_scripts_SSL4EO.get_args_finetune import get_args_dfc2020 as get_args
from models.pl_finetune import UNET_PL_FT

# dataset
from dataset.dfc2020_dataset_lmdb import DFC2020Dataset, SegAugTransforms
...
def main():
    ...
    if args.mt.startswith('vit'):
        ...
        trans_tr = A.Compose([# A.RandomCrop(width=256, height=256),
                              A.Flip(p=0.5),
                              A.Rotate(p=0.2),
                             ])
        trans_ts = None
    def get_dataset(data_path, trans=None):
        dataset = DFC2020Dataset(data_path, mode=args.it, n_bands=args.n_channels, 
    ...
    # if args.n_classes<8: args.n_classes = args.n_classes+1 # add one class for the background
    ...
    callbacks = [ModelCheckpoint(dirpath = args.checkpoints_dir, every_n_epochs = args.save_inv, 
                                 save_top_k=-1,  # Save all models  #  save_weights_only=True, 
                                 )]
