...
SDTYPE = {1:np.float64, 2:np.uint16}
...
class DFC2020Dataset(Dataset):
    def __init__(self, lmdb_file, mode='s2', n_bands=13, transform=None, s1_transform=None, s2_transform=None, 
    ...
    def __getitem__(self, index):
        ...
        with self.env.begin(write=False) as txn:
            # read labels
            key_lbl = f'lbl_{index}'
            data_lbl = txn.get(key_lbl.encode())
            lbl_bytes, lbl_shape = pickle.loads(data_lbl)
            ...
            for s in [1, 2]:
                if str(s) in self.mode: 
                    ...
                    img_s, shape_s = pickle.loads(data_s)
        ...
        if '1' in self.mode:
            ...
            if self.s1_normalize:
                # normalize by /10000
                img = data_normalize_s1(img).astype(np.float32)
            else:
...
if __name__ == '__main__':
    selected_i = 0
    si = 2
    df_val = '/p/project/hai_dm4eo/liu_ch/data/DFC2020/DFC2020_s12_validation.lmdb'
    df_test = '/p/project/hai_dm4eo/liu_ch/data/DFC2020/DFC2020_s12_test.lmdb'
