...
class UNET_PL_FT(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if self.schdt=='none':
            ...
            if self.schdt=='step':
                # if self.eft_tag=='eft': 
                #     tmp = np.array(self.schdv)>self.eft_ep
                #     if np.sum(tmp)>0:
                #         print(f'Some epoch settings for scheduler ({self.schdv}) are beyond the epoch for encoder unfixing ({self.eft_ep}), they are removed!')
                #         self.schdv = self.schdv[:len(tmp)-np.sum(tmp)]
                ...
                gamma = self.schdv[0]
                if len(self.schdv)>1:
                    print(f'More values than one ({self.schdv}) are provided for exponential decay scheduler, only the first one ({gamma}) is used!')
                scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma)
            # # for encoder finetuning
            # if self.eft_tag=='eft':
            #     scheduler_ft = torch.optim.lr_scheduler.MultiStepLR(optimizer, [self.eft_ep], gamma=1)
            #     scheduler = torch.optim.lr_scheduler.SequentialLR(optimizer, schedulers=[scheduler_, scheduler_ft], milestones=[self.eft_ep])
            # else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.eft_tag=='eft' and self.current_epoch==self.eft_ep-1 and batch_idx==0:
    ...
    def _get_loss(self, loss_type, n_classes, if_mask):
        ...
        if 'd' in loss_type:
            self.DLoss = DiceLoss(mode='multiclass' if n_classes>1 else 'binary', 
                                  from_logits=True, 
                                  ignore_index=255 if if_mask else None)
    ...
    def _get_batch_acc(self, pred_logits, y, label_type='tr'):
        ...
        metrics = evl.evaluate_batch_pl(pred_logits, y, self.n_classes, mask_id=255 if self.if_mask else None)
        for mk in metrics:
            metr = metrics[mk]
            if torch.prod(torch.tensor(metr.size()))>1:
                for i in range(len(metr)-1):
    ...
    def _get_batch_conf_matrix(self, pred_logits, y, if_val=True):
        metrics = evl.get_confuse_matrix_batch_pl(pred_logits, y, self.n_classes, mask_id=255 if self.if_mask else None)
        for mk in metrics:
            if if_val:
